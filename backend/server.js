...
// PORT: 3000
// DATABASE: databases/salon.db ONLY
// USAGE: Merchant Portal bookings, availability, metrics
// DO NOT USE merchant.db - that's for frontend/backend/server.js (port 5000)
...
import "./config/tz-setup.js";
import { db } from "./config/db.js"; // Uses databases/salon.db
import { generateSlots } from "./services/availability.js";
import { handleInboundMessage } from "./handlers/messageHandler.js";
import { startWweb, getWwebClient } from "./services/whatsapp.js";
import { salonHours } from "./config/rules.js";
...
// --- Shared API auth for Merchant Portal (token-based) ---
function portalAuth(req, res, next) {
  const expected = process.env.PORTAL_API_KEY;
  if (!expected) {
    // If no key configured, allow for local/dev usage
    return next();
  }
...
// ---- Shared booking/availability API (read-only, token protected) ----
const qBookingsRange = db.prepare(`
  SELECT 
    b.id,
    b.phone,
    b.service_id,
...
const qBookingForApi = db.prepare(`
  SELECT 
    b.id,
    b.phone,
    b.service_id,
...
const qCancelPortal = db.prepare(
  "UPDATE bookings SET status='cancelled', hold_until=NULL WHERE id=? AND status!='cancelled'"
);
const qUpdateBookingTimeStaff = db.prepare(
  "UPDATE bookings SET staff_id=?, start_dt=?, end_dt=? WHERE id=?"
);

function normalizePortalBooking(row, now) {
  const startDt = dayjs(row.start_dt);
  const endDt = dayjs(row.end_dt);
  const durationMin = Math.max(0, endDt.diff(startDt, "minute"));

  // Map internal status → portal status
  let portalStatus = row.status;
  if (row.status === "confirmed" && endDt.isBefore(now)) {
...
app.get("/api/v1/bookings", portalAuth, (req, res) => {
  let { start, end, status } = req.query || {};
  const today = dayjs().startOf("day");

  // Default range: ALL bookings (past and future) if no date range specified
  // This allows merchant portal to see all bookings in salon.db
  if (!start || !/^\d{4}-\d{2}-\d{2}$/.test(String(start))) {
    // Get earliest booking date or default to 1 year ago
    try {
      const earliestBooking = db.prepare("SELECT MIN(date(start_dt)) as earliest FROM bookings").get();
      start = earliestBooking?.earliest || today.subtract(365, "day").format("YYYY-MM-DD");
    } catch (e) {
      start = today.subtract(365, "day").format("YYYY-MM-DD");
    }
  }
  if (!end || !/^\d{4}-\d{2}-\d{2}$/.test(String(end))) {
    // Get latest booking date or default to 1 year in future
    try {
      const latestBooking = db.prepare("SELECT MAX(date(start_dt)) as latest FROM bookings").get();
      end = latestBooking?.latest || today.add(365, "day").format("YYYY-MM-DD");
    } catch (e) {
      end = today.add(365, "day").format("YYYY-MM-DD");
    }
  }

  try {
    const rows = qBookingsRange.all(start, end);
    const now = dayjs();
    const normalized = rows
      ...
      .map((row) => normalizePortalBooking(row, now));

    res.json({ start, end, count: normalized.length, bookings: normalized });
  } catch (e) {
    console.error("api/v1/bookings error", e);
    res.status(500).json({ error: "internal_error" });
  }
});

// Cancel a booking (portal)
app.post("/api/v1/bookings/:id/cancel", portalAuth, (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) {
    return res.status(400).json({ error: "invalid_id" });
  }

  const existing = qBookingForApi.get(id);
  if (!existing) return res.status(404).json({ error: "not_found" });

  qCancelPortal.run(id);
  const updated = qBookingForApi.get(id);
  const booking = normalizePortalBooking(updated || existing, dayjs());
  return res.json({ ok: true, booking });
});

// Reschedule a booking (portal, simple overwrite)
app.post("/api/v1/bookings/:id/reschedule", portalAuth, (req, res) => {
  const id = Number(req.params.id);
  const { start, staff_id } = req.body || {};
  if (!Number.isFinite(id)) {
    return res.status(400).json({ error: "invalid_id" });
  }
  if (!start) {
    return res.status(400).json({ error: "start_required" });
  }

  const row = qBookingForApi.get(id);
  if (!row) return res.status(404).json({ error: "not_found" });

  const svc = qGetSvcById.get(row.service_id);
  if (!svc) return res.status(404).json({ error: "service_not_found" });

  const staffId = Number(staff_id || row.staff_id);
  const stf = qGetStaffById.get(staffId);
  if (!stf) return res.status(404).json({ error: "staff_not_found" });
  ...
  try {
    const tx = db.transaction(() => {
      const clash = qClash.get(staffId, startISO);
      if (clash) throw new Error("slot_taken");
      qUpdateBookingTimeStaff.run(staffId, startISO, endISO, id);
    });
    tx();
  } catch (e) {
    ...
    console.error("api/v1/bookings/:id/reschedule error", e);
    return res.status(500).json({ error: "internal_error" });
  }

  const updated = qBookingForApi.get(id);
  const booking = normalizePortalBooking(updated, dayjs());
  return res.json({ ok: true, booking });
});
...
const server = app.listen(PORT, () => {
    console.log(`🚀 Main Backend Server listening on http://localhost:${PORT}`);
    console.log(`📁 Using database: databases/salon.db`);
    console.log(`✅ Ready to serve Merchant Portal (salon.db only)`);
});
