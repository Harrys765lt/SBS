...
export function BookingsPage({ onNavigate }: BookingsPageProps) {
  ...
  React.useEffect(() => {
    const fetchBookings = async () => {
      try {
        setLoading(true);
        // Fetch from main backend API (port 3000)
        // No date range = get ALL bookings from salon.db
        const response = await fetch('http://localhost:3000/api/v1/bookings?status=all');
        ...
        if (!contentType || !contentType.includes('application/json')) {
          ...
          throw new Error('API returned non-JSON response. Is the backend server running on port 3000?');
        }
        ...
        if (data.bookings && Array.isArray(data.bookings)) {
          ...
          const transformedBookings: Booking[] = data.bookings.map((b: any) => ({
            ...
            price: 0, // Price not available in API response
          ...
          // API returned but no bookings array - set empty array
        ...
        // Show empty array instead of mock data to avoid confusion
        // This ensures users know they're not seeing real data
        setBookings([]);
        setError('Failed to load bookings from database. Please ensure the backend server is running on port 3000.');
      } finally {
        setLoading(false);
      }
    };
              ...
              {paginatedBookings.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={10} className="text-center py-8 text-muted-foreground">
                    {loading ? 'Loading bookings...' : error ? error : 'No bookings found. Bookings from salon.db will appear here.'}
                  </TableCell>
                </TableRow>
              ) : (
